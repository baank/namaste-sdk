 ...
 */
package com.naden.namaste.osgi
...
class ServicesWatcher[I <: AnyRef](
    interface: Class[I],
  ...
  def andHandle(handler: PartialFunction[ServiceEvent[I], Unit]) {
    ...
    val tracker = new ServiceTracker[I, I](context, context createFilter fullFilter.toString, null) {

      override def addingService(serviceReference: ServiceReference[I]) = {
        val service = context getService serviceReference.asInstanceOf[ServiceReference[I]]
        val serviceEvent = AddingService(service, serviceReference.properties)
        if (handler.isDefinedAt(serviceEvent)) {
          handler(serviceEvent)
          println("Handled AddingService event.")
        }
        service
      }

      override def modifiedService(serviceReference: ServiceReference[I], service: I) {
        val serviceEvent = ServiceModified(service.asInstanceOf[I], serviceReference.properties)
        if (handler.isDefinedAt(serviceEvent)) {
          handler(serviceEvent)
          println("Handled ServiceModified event.")
        }
      }

      override def removedService(serviceReference: ServiceReference[I], service: I) {
        val serviceEvent = ServiceRemoved(service.asInstanceOf[I], serviceReference.properties)
        if (handler.isDefinedAt(serviceEvent)) {
          handler(serviceEvent)
          println("Handled ServiceRemoved event.")
        }
