...
class ServiceFinder[I <: AnyRef](interface: Class[I], context: BundleContext) {
  ...
  def andApply[T](f: I => T): Option[T] = {
    ...
    context getServiceReference interface.getName match {
      case null => {
        println("Could not find a ServiceReference for interface %s.".format(interface.getName))
        None
      }
      case ref => {
        println("Found a ServiceReference for interface %s.".format(interface.getName))
...
class ServicesFinder[I <: AnyRef](
  ...
  def andApply[T](f: I => T): Seq[T] = {
    ...
    context.getServiceReferences(interface.getName, filter map {
      ...
      case null => {
        println("Could not find any ServiceReferences for interface %s and optional filter %s.".format(interface.getName, filter))
        Nil
      }
      case refs => {
        println("Found %s ServiceReferences for interface %s and optional filter %s.".format(refs.size, interface.getName, filter))
        refs.toList.map(_.asInstanceOf[ServiceReference[I]]) flatMap {
          invokeService(_, f, context)
        }
      }
    }
  }
