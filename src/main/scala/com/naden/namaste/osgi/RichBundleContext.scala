...
class RichBundleContext(context: BundleContext) {
  ...
  def createService[S <: AnyRef, I1 >: S <: AnyRef, I2 >: S <: AnyRef, I3 >: S <: AnyRef](
    ...
    val serviceRegistration = context.registerService(interfaces, service, if (properties.isEmpty) null else properties)
    println("Created service %s with interfaces %s and properties %s.".format(service, interfaces, properties))
    serviceRegistration.asInstanceOf[ServiceRegistration[S]]
  }
