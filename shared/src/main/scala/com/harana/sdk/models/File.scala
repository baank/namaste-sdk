...
object File {
	...
	def apply(name: String, description: String, fileName: String, fileSize: Long, mimeType: String, createdBy: User): File = {
		apply(name, description, fileName, fileSize, mimeType, createdBy.id, Instant.now, createdBy.id, Instant.now, None, Status.Active, 1L, Map())
	}
}